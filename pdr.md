...
- **Strategia di Merge (sync successive)**: Per le sincronizzazioni automatiche successive alla prima, viene confrontato `updatedAt` per ogni elemento: vince il più recente tra locale e cloud. La cancellazione di un elemento (soft delete con `deleted: true`) aggiorna anche `updatedAt`, garantendo che la versione locale cancellata sia riconosciuta come più recente rispetto a quella cloud non cancellata e venga correttamente propagata.
- **Auto-sync Silenziosa (Token Persistence + Invisible Refresh)**: All'avvio dell'app, il token di accesso OAuth viene cercato prima in `localStorage` (dove è persistito insieme al suo timestamp di scadenza). Se il token è ancora valido, viene usato immediatamente per avviare la sincronizzazione in background senza alcuna interazione visibile. Se il token risulta scaduto (es. l'app viene riaperta il giorno dopo), il sistema utilizza `gapi.auth.authorize` con `immediate: true` per ottenere un nuovo token in modo completamente invisibile tramite un **iframe nascosto** gestito internamente dalla libreria GAPI. Questo approccio non apre alcun popup, finestra o overlay visibile — l'intera operazione avviene in background senza che l'utente percepisca nulla. Se il refresh silenzioso fallisce, l'app non mostra errori né popup ma indica "Tocca Sincronizza per aggiornare" nelle impostazioni. Per quanto riguarda i salvataggi, l'app utilizza una strategia di **Fast Sync**: non attende secondi preziosi né aspetta la chiusura dell'app (azioni facilmente interrotte dal sistema operativo sui dispositivi mobile chiudendo preventivamente i thread o i Service Worker postMessage). Al contrario, ogni qualvolta avviene una modifica (scrittura, eliminazione, spunta), dopo soli **500 millisecondi** l'app esegue una richiesta diretta `PATCH` in background ai server Google aggiornando il file istantaneamente (senza effettuare le lente operazioni di lettura + merge che richiederebbero più richieste API sequenziali). Questa velocità garantisce che il dato sia già arrivato al cloud prima ancora che l'utente compia il gesto meccanico di chiusura dell'app. La procedura `syncOnClose()` subentra solo come estremo backup congiunto a `Service Worker` in caso di chiusure fulminee al di sotto della soglia dei 500ms.
