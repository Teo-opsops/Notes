...
- **Auto-sync Silenziosa (Token Persistence + Invisible Refresh)**: All'avvio dell'app, il token di accesso OAuth viene cercato prima in `localStorage` (dove è persistito insieme al suo timestamp di scadenza). Se il token è ancora valido, viene usato immediatamente per avviare la sincronizzazione in background senza alcuna interazione visibile. Se il token risulta scaduto (es. l'app viene riaperta il giorno dopo), il sistema utilizza `gapi.auth.authorize` con `immediate: true` per ottenere un nuovo token in modo completamente invisibile tramite un **iframe nascosto** gestito internamente dalla libreria GAPI. Questo approccio non apre alcun popup, finestra o overlay visibile — l'intera operazione avviene in background senza che l'utente percepisca nulla. Se il refresh silenzioso fallisce (es. sessione Google scaduta, cookie di terze parti bloccati), l'app non mostra errori né popup: semplicemente indica "Tocca Sincronizza per aggiornare" nello stato sync, lasciando all'utente la possibilità di sincronizzare manualmente dalle impostazioni (dove il popup è accettabile perché user-initiated). La funzione `ensureToken()` non richiede mai un nuovo token autonomamente (nessun popup a catena): verifica solo quello già in memoria e, se scaduto, lo rimuove silenziosamente. Ogni modifica locale avvia un timer di 15 secondi al termine del quale parte la sync (solo se il token è disponibile). Quando l'utente chiude l'app o passa a un'altra scheda (`visibilitychange` + `beforeunload`), se ci sono modifiche in sospeso l'app esegue una **sincronizzazione diretta "fire-and-forget"**: forza immediatamente il caricamento del file su Drive ignorando il pre-fetching e utilizzando l'opzione `keepalive: true` nel `fetch`, che garantisce che il browser completi l'upload in background anche se la scheda dell'app è già stata chiusa.
