...
- **Auto-sync Silenziosa**: La sincronizzazione parte in totale autonomia grazie al ripristino silenzioso del token all'avvio dell'applicazione. Il `tokenClient` è configurato con `login_hint` (email dell'utente salvata localmente) e `error_callback` per garantire che il rinnovo del token avvenga senza mostrare alcun popup o schermata di consenso all'utente, nemmeno brevemente. In caso di errore silenzioso del token (popup bloccato, sessione scaduta), non viene mostrato alcun errore e l'utente può sincronizzare manualmente o al prossimo accesso. Lo stato della sincronizzazione nelle impostazioni viene aggiornato anche per le sync silenziose e di avvio, mostrando sempre "Sincronizzato ✓" dopo un esito positivo. Ogni modifica locale avvia un timer di 15 secondi al termine del quale parte la sync (solo se il token è disponibile). La sync interviene istantaneamente anche quando l'utente chiude l'app o passa a un'altra scheda/app (`visibilitychange` + `beforeunload`), sempre con verifica della disponibilità del token.
